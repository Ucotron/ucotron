...
import { GitFork, Filter, X, ChevronRight, Loader2, AlertCircle, Search } from "lucide-react";
...
export default function GraphPage() {
  ...
  const [filteredNodeIds, setFilteredNodeIds] = useState<Set<number> | null>(null);
  ...
  useEffect(() => {
    ...
    const matchedIds = nodes
      ...
      .map((n) => n.id);
    setFilteredNodeIds(new Set(matchedIds));

    // Auto-select first matched node.
    if (matchedIds.length > 0 && selectedNodeId === null) {
      setSelectedNodeId(matchedIds[0]);
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-48 rounded-md border border-border bg-card px-9 py-1.5 text-sm placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
            />
            {searchQuery && (
              <button
          ...
          {filteredNodeIds && filteredNodeIds.size > 0 && (
            <span className="text-sm text-primary">
              {filteredNodeIds.size} match{filteredNodeIds.size !== 1 ? "es" : ""}
            </span>
          )}
          {filteredNodeIds && filteredNodeIds.size === 0 && searchQuery && (
            <span className="text-sm text-destructive">No matches</span>
          )}
